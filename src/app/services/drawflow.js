import {Node3Component} from "../components/node3/node3.component";
...
export default class Drawflow {
      ...
      if (elemsOut[item].querySelector('.point') === null) {
        ...
        points.forEach((item, i) => {
          if (i === 0 && points.length - 1 === 0) {
            ...
            var line_x =
              (elemtsearchId_out.getBoundingClientRect().x - precanvas.getBoundingClientRect().x) * precanvasWitdhZoom +
              rerouteWidth;
            var line_y =
              (elemtsearchId_out.getBoundingClientRect().y - precanvas.getBoundingClientRect().y) *
              precanvasHeightZoom +
              rerouteWidth;
            var x = eX;
            var y = eY;
            ...
            var eY =
              ...
              (elemtsearchIn.getBoundingClientRect().y - precanvas.getBoundingClientRect().y) * precanvasHeightZoom;
            var line_x =
              (elemtsearchId_out.getBoundingClientRect().x - precanvas.getBoundingClientRect().x) *
  ...
  addNodeImport(dataNode, precanvas) {
    ...
    if (dataNode.typenode === false) {
      ...
      const target = document.getElementById(this.getUuid.toString());
      if (target) {
        const componentRef = this.render.createComponent(Node3Component);
        target.appendChild(componentRef.location.nativeElement);
      }
    } else {
      if (parseInt(this.render.version) === 3) {
